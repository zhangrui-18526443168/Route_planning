% 将必经节点联结成无间断路径，如果节点间不连续，则插入节点使其连续
...
while i ~= single_path_num
    % 定位第i，i+1个节点的坐标
    column_now = mod(single_new_pop(1, i), x) + 1;  % 路径中第i个栅格在地图的列（从左到右编号1.2.3...）
    row_now = fix(single_new_pop(1, i) / x) +1;     % 路径中第i个栅格在地图的行（从上到下编号1.2.3...）
    ...
    while max(abs(column_next - column_now), abs(row_next - row_now)) ~= 1
        % 取两节点的中心做为插入点，插入节点的横坐标x_insert 纵坐标y_insert
        x_insert = floor((column_next + column_now) / 2);
        y_insert = floor((row_next + row_now) / 2);

        % 插入栅格为自由栅格
        if Grid(y_insert, x_insert) == 0
            num_insert = (x_insert - 1) + (y_insert - 1) * x;  % 插入的栅格序号
            single_new_pop = [single_new_pop(1, 1:i), num_insert, single_new_pop(1, i+1 : end)];  % 插入新序号(将当前的栅格序号中间插入一个新栅格序号，其他保持不变)
            % 插入栅格为障碍物栅格
        else
            % 往左走(如果当前待插入格（障碍物格)的左邻格不是障碍物 且 左邻格不是当前的两个格中任意一个)
            if Grid(y_insert, x_insert - 1) == 0 && ((x_insert - 2) + (y_insert - 1) * x ~= single_new_pop(1, i)) && ((x_insert - 2) + (y_insert - 1) * x ~= single_new_pop(1, i + 1))
                x_insert = x_insert - 1;
                num_insert = (x_insert - 1) + (y_insert - 1) * x;  % 栅格序号
